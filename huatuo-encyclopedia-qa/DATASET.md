...
**重要：训练集、测试集、验证集必须分开成3个独立的Release文件上传！**

每个子集独立发布，便于用户按需下载：

| Release 标签 | 对应数据 | 说明 |
|--------------|----------|------|
| `v1.0-train` | 训练集处理后数据 | 完整训练数据 (~36万条) |
| `v1.0-test` | 测试集处理后数据 | 完整测试数据 (~1000条) |
| `v1.0-validation` | 验证集处理后数据 | 完整验证数据 (~1000条) |

**注意**：
- 不要合并成一个Release
- 每个Release只包含对应子集的数据文件
- 命名格式：`huatuo_{split}_part_{xxx}.jsonl`
