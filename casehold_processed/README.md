...
casehold_processed/
├── README.md                          # 本文件
├── train_triplet.jsonl.gz            # 训练集 - 三联体格式 (压缩)
├── validation_triplet.jsonl          # 验证集 - 三联体格式
├── train_instruction.jsonl.gz        # 训练集 - 指令格式 (压缩)
├── validation_instruction.jsonl      # 验证集 - 指令格式
├── train_contrastive.jsonl.gz        # 训练集 - 对比学习格式 (压缩)
├── validation_contrastive.jsonl      # 验证集 - 对比学习格式
└── scripts/
    └── process_casehold.py           # 数据处理脚本
```

**注意**: 训练集文件 (>100MB) 已压缩为 `.gz` 格式，验证集保持原样 (<100MB)
